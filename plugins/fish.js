...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.DATABASE._data.users[m.sender].healt > 79) {
            if (new Date - global.DATABASE._data.users[m.sender].lastfishing > 1000) {
            ...
            let str = `
Nyawa mu berkurang -${healt * 1} karena Kamu telah memancing sampai ${pickRandom(['Afrika', 'Samudra', 'Lava', 'Indonesia', 'Iraq', 'Arab', 'Pakistan', 'German', 'Finlandia', 'Ke bawa dunia mimpi', 'Ujung dunia', 'Mars', 'Bulan', 'Pluto', 'Matahari', 'Hatinya dia', '...'])} dan mendapatkan
*exp:* ${exp} 
*uang:* ${uang}
...
`.trim()
            ...
            if (mythic > 0) {
                   ...
                   conn.reply(m.chat, '*Selamat anda mendapatkan Harta karun Rare yaitu*\n' + mythic + ' Mythic Crate', m)
            }
            if (legendary > 0) {
                global.DATABASE._data.users[m.sender].legendary += legendary * 1
                conn.reply(m.chat, '*Selamat anda mendapatkan Harta karun Epic yaitu*\n' + legendary + ' Legendary Crate', m)
            }
            ...
            } else conn.reply(m.chat, `Anda sudah memancing dan kelelahan, silahkan coba *${timers}* lagi`, m)
